...
#if PAMP_CHANGES
// PAMP Change: downgrade decoder error logs to warnings (avoid error spam for tolerated/recoverable AAC stream issues)
...
#if PAMP_CHANGES && CONFIG_SMALL
// PAMP Change: profiles.c only defines ff_aac_profiles when !CONFIG_SMALL; PA builds with
// CONFIG_SMALL=1, so define it here (no duplicate symbol) to keep .profiles non-NULL for get_codec_name().
...
#if PAMP_CHANGES
    .p.profiles      = ff_aac_profiles, // PAMP Change: needed for get_codec_name (kept non-NULL under CONFIG_SMALL)
#else
