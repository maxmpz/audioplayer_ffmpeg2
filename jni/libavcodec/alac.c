...
static void lpc_prediction(int32_t * __restrict__ error_buffer, uint32_t * __restrict__ buffer_out,
                          ...
                          const int channels)
{
    ...
    if (is_compressed) {
        ...
        if (alac->extra_bits) {
            ...
            if (get_bits_left(gb) < (int64_t)nb_samples * channels * extra_bits)
                return AVERROR_INVALIDDATA;
            for (i = 0; i < nb_samples; i++) {
                #pragma clang loop unroll_count(2)
                for (ch = 0; ch < channels; ch++)
        ...
        for (ch = 0; ch < channels; ch++) {
            ...
            if (prediction_type[ch] == 15) {
                ...
                lpc_prediction(alac->predict_error_buffer[ch],
                               (uint32_t *)alac->predict_error_buffer[ch],
            ...
            lpc_prediction(alac->predict_error_buffer[ch],
                           (uint32_t *)alac->output_samples_buffer[ch], nb_samples,
                           bps, lpc_coefs[ch], lpc_order[ch], lpc_quant[ch]);
        }
    } else {
        const int sample_size = alac->sample_size;
        /* not compressed, easy case */
        if (get_bits_left(gb) < (int64_t)nb_samples * channels * sample_size)
            return AVERROR_INVALIDDATA;
        for (i = 0; i < nb_samples; i++) {
            for (ch = 0; ch < channels; ch++) {
    ...
    if (channels == 2) {
        ...
        if (decorr_left_weight) {
            alac->dsp.decorrelate_stereo(alac->output_samples_buffer, nb_samples,
