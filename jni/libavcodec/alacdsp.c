                               ...
                               int decorr_shift, int decorr_left_weight)
{
    ...
    for (i = 0; i < cnb_samples; i++) {
        ...
        a -= (int)(b * cdecorr_left_weight) >> cdecorr_shift;
        b += a;
                              ...
                              int extra_bits, int channels, int nb_samples)
{
    ...
    if(channels == 2) {
        ...
        for (int i = 0; i < cnb_samples; i++) {
            buffer0[i] = ((unsigned)buffer0[i] << cextra_bits) | extra_bits_buffer0[i];
            buffer1[i] = ((unsigned)buffer1[i] << cextra_bits) | extra_bits_buffer1[i];
        }

    } else if(channels == 1) {
        for (int ch = 0; ch < cchannels; ch++) {
            for (int i = 0; i < cnb_samples; i++) {
                buffer[ch][i] = ((unsigned)buffer[ch][i] << cextra_bits) | extra_bits_buffer[ch][i]; // MaxMP: this will fail for # of channels > 2
            }
        }
    }
}
