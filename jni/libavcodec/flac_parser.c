...
#if PAMP_CHANGES // Pamp change: captured max_blocksize from file header STREAMINFO (priv_data/FLACContext is not reliably available here)
    int max_blocksize;             /**< STREAMINFO max blocksize, or -1 if invalid, 0 if not yet captured */
#endif
} FLACParseContext;
...
#if PAMP_CHANGES
        // MaxMP: always check for best_child, otherwise we're getting bad header here for some rare flacs
        if (header->best_child) {
            if (header->fi.is_var_size) {
                ...
                // As parser technically can be used w/o decoder data, we use it only if it's available (captured from extradata STREAMINFO).
                ...
                DLOG2("%s #2 fpc=%p avctx=%p frame_or_sample_num=%" PRId64 " fi.blocksize=%d blocksize=%d pts=>%" PRId64,
                      __FUNC__, fpc, fpc->avctx, header->fi.frame_or_sample_num, header->fi.blocksize, blocksize, fpc->pc->pts);
            }
        }
#else
        if (header->fi.is_var_size)
...
#if PAMP_CHANGES
    // Pamp change: capture STREAMINFO max_blocksize from extradata once. We use it in get_best_header() to compute a stable pts for
    // fixed-block-size streams whose last frame reports a smaller blocksize. We cannot rely on FLACContext (decoder priv_data) being
    // available to the parser, so capture it directly from extradata here.
