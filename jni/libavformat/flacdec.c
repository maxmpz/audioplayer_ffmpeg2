...
static int flac_read_header(AVFormatContext *s)
{
    ...
    while (!avio_feof(s->pb) && !metadata_last) {
...
#if PAMP_CONFIG_NO_TAGS // PAMP change: some flacs can contain an image larger than the possible flac metadata size (24bit length => 16mb).
        case FLAC_METADATA_TYPE_PICTURE: {
            // Try to parse FLAC_METADATA_TYPE_PICTURE looking for imgSize, then skip it.
            // But if for any reason we seeked less than metadata_size, ensure we fully skip the current metadata block.
...
#if !PAMP_CONFIG_NO_TAGS // PAMP change: PICTURE handled above when tags disabled
        case FLAC_METADATA_TYPE_PICTURE:
#endif // End PAMP change
        ...
        if (metadata_type == FLAC_METADATA_TYPE_STREAMINFO) {
            ...
            if (found_streaminfo) {
#if !PAMP_CHANGES // PAMP change: don't error - there are files like that in the wild (815-flac-not-played)
...
#if !PAMP_CONFIG_NO_TAGS // PAMP change: picture already skipped above when tags disabled (buffer is NULL)
...
#if !PAMP_CONFIG_NO_TAGS // PAMP change: skip replaygain export when tags disabled
...
#if PAMP_CHANGES // PAMP change (OLD a18810c): avoid rare SIGSEGV in av_parser_parse2 on a NULL packet buffer during FLAC seek.
        // pkt->data can be NULL; allow size==0 with !buf, otherwise require a real buf before parsing (size stays 0 -> loop falls through).
        if (pkt->size == 0 || pkt->buf)
#endif
