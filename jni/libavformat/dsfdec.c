...
#if PAMP_CHANGES // PAMP change: don't read last block as it sometimes contains trash for some reason
    if (dsf->data_end > st->codecpar->block_align) {
        dsf->data_end  -= st->codecpar->block_align;
        dsf->data_size -= st->codecpar->block_align;
    }
