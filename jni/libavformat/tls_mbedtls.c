...
#if CONFIG_DTLS_PROTOCOL // PAMP: DTLS/WebRTC cert-gen helpers need mbedtls 3.x APIs (mbedtls_x509write_crt_*, mbedtls_sha256) + MBEDTLS_X509_CRT_WRITE_C. PA is HTTPS-client-only (DTLS disabled) on vendored mbedtls 2.17.0 - guard them out.
...
#endif // CONFIG_DTLS_PROTOCOL (PAMP: DTLS/WebRTC cert-gen helpers)
...
// MaxMP: DEBUG hook. NOTE: requires mbedtls MBEDTLS_DEBUG_C to be enabled in the mbedtls config to actually fire
...
static int mbedtls_recv(void *ctx, unsigned char *buf, size_t len)
{
    ...
    if (ret >= 0) {
#if CONFIG_DTLS_PROTOCOL // PAMP: DTLS-over-UDP listen path uses ff_udp_* (udp protocol not built in PA); PA HTTPS-client-only
...
static int tls_open(URLContext *h, const char *uri, int flags, AVDictionary **options)
{
    ...
    (void)f_dbg; // MaxMP: suppress unused warning while debug hook is disabled
#endif
...
#if CONFIG_DTLS_PROTOCOL // PAMP: DTLS server ephemeral cert-gen (mbedtls 3.x); PA HTTPS-client-only on mbedtls 2.17.0
...
#if CONFIG_DTLS_PROTOCOL // PAMP: DTLS config (mbedtls DTLS APIs absent in PA's mbedtls 2.17.0 build); PA HTTPS-client-only
    if (shr->is_dtls) {
...
#if CONFIG_DTLS_PROTOCOL // PAMP: DTLS timers/MTU; PA HTTPS-client-only on mbedtls 2.17.0
    if (shr->is_dtls) {
