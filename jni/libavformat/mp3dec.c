...
#if PAMP_CHANGES // PAMP change: >>4 => /2 - increase possible delta needed for using bitrate for duration. REVISIT: this should also account for images, which can be any size
        if (fsize > mp3->header_filesize && delta > min / 2) {
            mp3->frames = 0;
            av_log(s, AV_LOG_WARNING,
                   "invalid concatenated file detected - using bitrate for duration frames=%u fsize=%" PRIu64 " header_filesize=%u delta=%" PRIu64 " min=%" PRIu64 " min/2=%" PRIu64 "\n",
...
#if !PAMP_CONFIG_NO_TAGS // PAMP change: skip replaygain/encoder tag export when tags disabled
    if (v == crc) {
...
#if !PAMP_CONFIG_NO_TAGS // PAMP change: skip id3v1 tag read when tags disabled
