    ...
    for (;;) {
        ...
        case MKTAG('l', 'i', 's', 't'):
            if (size < 4) {
                ...
                //return AVERROR_INVALIDDATA; // Pamp change: don't fail even if list tag is invalid
            } else {
            switch (avio_rl32(pb)) {
            case MKTAG('I', 'N', 'F', 'O'):
            ...
            } // Pamp change: end of don't-fail else
            break;
        case MKTAG('I', 'D', '3', ' '):
        case MKTAG('i', 'd', '3', ' '): {
            ID3v2ExtraMeta *id3v2_extra_meta;
            ff_id3v2_read(s, ID3v2_DEFAULT_MAGIC, &id3v2_extra_meta, 0);
#if PAMP_CONFIG_NO_TAGS // Begin PAMP change - read only the id3v2 dict, skip apic/chapters/priv
