...
// Begin PAMP change: use NULL_IF_CONFIG_SMALL for options help labels (strips strings in CONFIG_SMALL builds).
// NOTE: 6.1 introduced functionally-defaulted options (use_tfdt=1, interleaved_read=1) and the use_mfra_for
// CONST sub-options that the 4.2 PA delta gated out under !PAMP_CHANGES did not exist; we KEEP all option
// entries so 6.1 runtime defaults/behavior (incl. fragmented-MP4 timestamps and demux interleaving) are honored.
