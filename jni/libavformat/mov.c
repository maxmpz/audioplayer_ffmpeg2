...
{
#if PAMP_CHANGES // Pamp change: NULL-safe, guard tfdt_dts
    if (frag_stream_info) {
...
#if PAMP_CHANGES
        } else if(strcmp(key, "iTunNORM") == 0 || strncasecmp(key, "replaygain_", 11) == 0) {
            av_log(NULL, AV_LOG_INFO, "mov meta=%s %s", key, val);
            av_dict_set(&c->fc->metadata, key, val, AV_DICT_DONT_STRDUP_VAL | AV_DICT_DONT_STRDUP_KEY);
            key = val = NULL;
...
static const AVOption mov_options[] = {
    ...
    {"seek_streams_individually",
        NULL_IF_CONFIG_SMALL("Seek each stream individually to the closest point"),
    ...
    {"advanced_editlist",
        ...
        0, 1, FLAGS},
    {"ignore_chapters", NULL_IF_CONFIG_SMALL(""), OFFSET(ignore_chapters), AV_OPT_TYPE_BOOL, {.i64 = 0},
        0, 1, FLAGS},
    {"use_mfra_for",
        ...
        .unit = "use_mfra_for"},
    {"auto", NULL_IF_CONFIG_SMALL("auto"), 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_MFRA_AUTO}, 0, 0,
        FLAGS, .unit = "use_mfra_for" },
    {"dts", NULL_IF_CONFIG_SMALL("dts"), 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_MFRA_DTS}, 0, 0,
        FLAGS, .unit = "use_mfra_for" },
    {"pts", NULL_IF_CONFIG_SMALL("pts"), 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_MFRA_PTS}, 0, 0,
        FLAGS, .unit = "use_mfra_for" },
    {"use_tfdt", NULL_IF_CONFIG_SMALL("use tfdt for fragment timestamps"), OFFSET(use_tfdt), AV_OPT_TYPE_BOOL, {.i64 = 1},
        0, 1, FLAGS},
    ...
    { "audible_key", NULL_IF_CONFIG_SMALL("AES-128 Key for Audible AAXC files"), OFFSET(audible_key),
        AV_OPT_TYPE_BINARY, .flags = AV_OPT_FLAG_DECODING_PARAM },
    { "audible_iv", NULL_IF_CONFIG_SMALL("AES-128 IV for Audible AAXC files"), OFFSET(audible_iv),
        AV_OPT_TYPE_BINARY, .flags = AV_OPT_FLAG_DECODING_PARAM },
    ...
    { "decryption_key", NULL_IF_CONFIG_SMALL("The default media decryption key (hex)"), OFFSET(decryption_default_key), AV_OPT_TYPE_BINARY, .flags = AV_OPT_FLAG_DECODING_PARAM },
    { "decryption_keys", NULL_IF_CONFIG_SMALL("The media decryption keys by KID (hex)"), OFFSET(decryption_keys), AV_OPT_TYPE_DICT, .flags = AV_OPT_FLAG_DECODING_PARAM },
    { "enable_drefs", NULL_IF_CONFIG_SMALL("Enable external track support."), OFFSET(enable_drefs), AV_OPT_TYPE_BOOL,
        {.i64 = 0}, 0, 1, FLAGS },
    { "max_stts_delta", NULL_IF_CONFIG_SMALL("treat offsets above this value as invalid"), OFFSET(max_stts_delta), AV_OPT_TYPE_INT, {.i64 = UINT_MAX-48000*10 }, 0, UINT_MAX, .flags = AV_OPT_FLAG_DECODING_PARAM },
    { "interleaved_read", NULL_IF_CONFIG_SMALL("Interleave packets from multiple tracks at demuxer level"), OFFSET(interleaved_read), AV_OPT_TYPE_BOOL, {.i64 = 1 }, 0, 1, .flags = AV_OPT_FLAG_DECODING_PARAM },
