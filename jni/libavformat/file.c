...
#if PAMP_CHANGES // PAMP change: optional debug logging (disabled by default)
...
#if PAMP_CHANGES // PAMP change: NULL_IF_CONFIG_SMALL drops help-text string literals from the binary when CONFIG_SMALL is set
    { "truncate", NULL_IF_CONFIG_SMALL("truncate existing files on write"), offsetof(FileContext, trunc), AV_OPT_TYPE_BOOL, { .i64 = 1 }, 0, 1, AV_OPT_FLAG_ENCODING_PARAM },
...
#if PAMP_CHANGES // PAMP change: NULL_IF_CONFIG_SMALL drops help-text string literals from the binary when CONFIG_SMALL is set; "seekable" added so pipe protocol honors the seekable property (see pipe_open)
    { "blocksize", NULL_IF_CONFIG_SMALL("set I/O operation maximum block size"), offsetof(FileContext, blocksize), AV_OPT_TYPE_INT, { .i64 = INT_MAX }, 1, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM },
    { "fd", NULL_IF_CONFIG_SMALL("set file descriptor"), offsetof(FileContext, fd), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM },
    { "seekable", NULL_IF_CONFIG_SMALL("Sets if the file is seekable"), offsetof(FileContext, seekable), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, 0, AV_OPT_FLAG_DECODING_PARAM | AV_OPT_FLAG_ENCODING_PARAM },
#else
    { "blocksize", "set I/O operation maximum block size", offsetof(FileContext, blocksize), AV_OPT_TYPE_INT, { .i64 = INT_MAX }, 1, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM },
...
static int pipe_open(URLContext *h, const char *filename, int flags)
{
    ...
    if (c->fd < 0) {
        ...
        if((filename == final)
#if !PAMP_CHANGES // PAMP change: ignore possible non-digits as we may pass some additional stuff like ?filename=filename.flac for better probing
        || *final
#endif
    ...
    if (c->fd == -1)
        return AVERROR(errno);
    h->is_streamed = 1;
#if PAMP_CHANGES // PAMP change: allow seekable property for pipe protocol.
    // 6.1 stock forces is_streamed=1 above; 4.2's pipe defaulted to seekable, so PA's seekable=-1 left it
    // seekable. The PA `seekable` option is only ever -1 (regular-file fd => want seeking) or 0 (real
    // pipe/socket => streamed), so map it directly. (The old `if (seekable >= 0) is_streamed = !seekable`
    // was a no-op for -1 under 6.1's is_streamed=1 default → broke ogg_get_length/duration for fd-opened files.)
    int orig_seekable = c->seekable;
    h->is_streamed = (c->seekable == 0);
