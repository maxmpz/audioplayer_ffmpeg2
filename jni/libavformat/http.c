...
#if PAMP_CHANGES
#define N(s) NULL_IF_CONFIG_SMALL(s) // Pamp change: strip option help strings in CONFIG_SMALL builds
#else
#define N(s) s
#endif
...
static const AVOption options[] = {
    ...
    { "reconnect_on_network_error", N("auto reconnect in case of tcp/tls error during connect"), OFFSET(reconnect_on_network_error), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, D },
    { "reconnect_on_http_error", N("list of http status codes to reconnect on"), OFFSET(reconnect_on_http_error), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    ...
    { "short_seek_size", N("Threshold to favor readahead over seek."), OFFSET(short_seek_size), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, INT_MAX, D },
    { NULL }
};
    ...
    if (!remaining) {
        ...
        if (ch > 0) {
            ...
            if (ret < 0)
                return ret;
#if PAMP_CHANGES
            data[len] = 0; // PAMP: fixed crash (was data[len + 1] = 0, off-by-one OOB write)
#else
            data[len + 1] = 0;
#endif
