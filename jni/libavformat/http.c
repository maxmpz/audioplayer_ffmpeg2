...
static const AVOption options[] = {
    { "seekable", NULL_IF_CONFIG_SMALL("control seekability of connection"), OFFSET(seekable), AV_OPT_TYPE_BOOL, { .i64 = -1 }, -1, 1, D },
    { "chunked_post", NULL_IF_CONFIG_SMALL("use chunked transfer-encoding for posts"), OFFSET(chunked_post), AV_OPT_TYPE_BOOL, { .i64 = 1 }, 0, 1, E },
    { "http_proxy", NULL_IF_CONFIG_SMALL("set HTTP proxy to tunnel through"), OFFSET(http_proxy), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D | E },
    { "headers", NULL_IF_CONFIG_SMALL("set custom HTTP headers, can override built in default headers"), OFFSET(headers), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D | E },
    { "content_type", NULL_IF_CONFIG_SMALL("set a specific content type for the POST messages"), OFFSET(content_type), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D | E },
    { "user_agent", NULL_IF_CONFIG_SMALL("override User-Agent header"), OFFSET(user_agent), AV_OPT_TYPE_STRING, { .str = DEFAULT_USER_AGENT }, 0, 0, D },
    { "referer", NULL_IF_CONFIG_SMALL("override referer header"), OFFSET(referer), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { "multiple_requests", NULL_IF_CONFIG_SMALL("use persistent connections"), OFFSET(multiple_requests), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, D | E },
    { "request_size", NULL_IF_CONFIG_SMALL("size (in bytes) of requests to make"), OFFSET(request_size), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, D },
    { "initial_request_size", NULL_IF_CONFIG_SMALL("size (in bytes) of initial requests made during probing / header parsing"), OFFSET(initial_request_size), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, D },
    { "post_data", NULL_IF_CONFIG_SMALL("set custom HTTP post data"), OFFSET(post_data), AV_OPT_TYPE_BINARY, .flags = D | E },
    { "mime_type", NULL_IF_CONFIG_SMALL("export the MIME type"), OFFSET(mime_type), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY },
    { "http_version", NULL_IF_CONFIG_SMALL("export the http response version"), OFFSET(http_version), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY },
    { "cookies", NULL_IF_CONFIG_SMALL("set cookies to be sent in applicable future requests, use newline delimited Set-Cookie HTTP field value syntax"), OFFSET(cookies), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { "icy", NULL_IF_CONFIG_SMALL("request ICY metadata"), OFFSET(icy), AV_OPT_TYPE_BOOL, { .i64 = 1 }, 0, 1, D },
    { "icy_metadata_headers", NULL_IF_CONFIG_SMALL("return ICY metadata headers"), OFFSET(icy_metadata_headers), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, AV_OPT_FLAG_EXPORT },
    { "icy_metadata_packet", NULL_IF_CONFIG_SMALL("return current ICY metadata packet"), OFFSET(icy_metadata_packet), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, AV_OPT_FLAG_EXPORT },
    { "metadata", NULL_IF_CONFIG_SMALL("metadata read from the bitstream"), OFFSET(metadata), AV_OPT_TYPE_DICT, {0}, 0, 0, AV_OPT_FLAG_EXPORT },
    { "auth_type", NULL_IF_CONFIG_SMALL("HTTP authentication type"), OFFSET(auth_state.auth_type), AV_OPT_TYPE_INT, { .i64 = HTTP_AUTH_NONE }, HTTP_AUTH_NONE, HTTP_AUTH_BASIC, D | E, .unit = "auth_type"},
    { "none", NULL_IF_CONFIG_SMALL("No auth method set, autodetect"), 0, AV_OPT_TYPE_CONST, { .i64 = HTTP_AUTH_NONE }, 0, 0, D | E, .unit = "auth_type"},
    { "basic", NULL_IF_CONFIG_SMALL("HTTP basic authentication"), 0, AV_OPT_TYPE_CONST, { .i64 = HTTP_AUTH_BASIC }, 0, 0, D | E, .unit = "auth_type"},
    { "send_expect_100", NULL_IF_CONFIG_SMALL("Force sending an Expect: 100-continue header for POST"), OFFSET(send_expect_100), AV_OPT_TYPE_BOOL, { .i64 = -1 }, -1, 1, E },
    { "location", NULL_IF_CONFIG_SMALL("The actual location of the data received"), OFFSET(location), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D | E },
    { "offset", NULL_IF_CONFIG_SMALL("initial byte offset"), OFFSET(off), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, D },
    { "end_offset", NULL_IF_CONFIG_SMALL("try to limit the request to bytes preceding this offset"), OFFSET(end_off), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, D },
    { "method", NULL_IF_CONFIG_SMALL("Override the HTTP method or set the expected HTTP method from a client"), OFFSET(method), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D | E },
    { "reconnect", NULL_IF_CONFIG_SMALL("auto reconnect after disconnect before EOF"), OFFSET(reconnect), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, D },
    { "reconnect_at_eof", NULL_IF_CONFIG_SMALL("auto reconnect at EOF"), OFFSET(reconnect_at_eof), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, D },
    { "reconnect_on_network_error", NULL_IF_CONFIG_SMALL("auto reconnect in case of tcp/tls error during connect"), OFFSET(reconnect_on_network_error), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, D },
    { "reconnect_on_http_error", NULL_IF_CONFIG_SMALL("list of http status codes to reconnect on"), OFFSET(reconnect_on_http_error), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { "reconnect_streamed", NULL_IF_CONFIG_SMALL("auto reconnect streamed / non seekable streams"), OFFSET(reconnect_streamed), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, D },
    { "reconnect_delay_max", NULL_IF_CONFIG_SMALL("max reconnect delay in seconds after which to give up"), OFFSET(reconnect_delay_max), AV_OPT_TYPE_INT, { .i64 = 120 }, 0, UINT_MAX/1000/1000, D },
    { "reconnect_max_retries", NULL_IF_CONFIG_SMALL("the max number of times to retry a connection"), OFFSET(reconnect_max_retries), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, D },
    { "reconnect_delay_total_max", NULL_IF_CONFIG_SMALL("max total reconnect delay in seconds after which to give up"), OFFSET(reconnect_delay_total_max), AV_OPT_TYPE_INT, { .i64 = 256 }, 0, UINT_MAX/1000/1000, D },
    { "respect_retry_after", NULL_IF_CONFIG_SMALL("respect the Retry-After header when retrying connections"), OFFSET(respect_retry_after), AV_OPT_TYPE_BOOL, { .i64 = 1 }, 0, 1, D },
    { "listen", NULL_IF_CONFIG_SMALL("listen on HTTP"), OFFSET(listen), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 2, D | E },
    { "resource", NULL_IF_CONFIG_SMALL("The resource requested by a client"), OFFSET(resource), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, E },
    { "reply_code", NULL_IF_CONFIG_SMALL("The http status code to return to a client"), OFFSET(reply_code), AV_OPT_TYPE_INT, { .i64 = 200}, INT_MIN, 599, E},
    { "short_seek_size", NULL_IF_CONFIG_SMALL("Threshold to favor readahead over seek."), OFFSET(short_seek_size), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, INT_MAX, D },
    { "max_redirects", NULL_IF_CONFIG_SMALL("Maximum number of redirects"), OFFSET(max_redirects), AV_OPT_TYPE_INT, { .i64 = MAX_REDIRECTS }, 0, INT_MAX, D },
    { NULL }
};
...
retry:
    ...
    while (read_ret < 0) {
        ...
        if (h->is_streamed && !s->reconnect_streamed) {
            DLOG("%s break !reconnect_streamed", __FUNC__);
            break;
        }

        if (!(s->reconnect && s->filesize > 0 && s->off < s->filesize) &&
            !(s->reconnect_at_eof && read_ret == AVERROR_EOF)) {
            DLOG("%s break !reconnect/!reconnect_at_eof", __FUNC__);
            break;
        }

        if (reconnect_delay > s->reconnect_delay_max) {
            //DLOG("%s break !reconnect_delay=%d", __FUNC__, reconnect_delay);
            //return AVERROR(EIO);
            reconnect_delay = s->reconnect_delay_max; // MaxMP: do not exit from here, just keep last reconnect delay
        }
        ...
        if (err != AVERROR(ETIMEDOUT)) {
            DLOG("%s break !err=%d", __FUNC__, err);
            return err;
        }
        reconnect_delay = 1 + 2*reconnect_delay;
        ...
        if (seek_ret >= 0 && seek_ret != target) {
            ...
            DLOG("%s break read_ret=%d", __FUNC__, read_ret);
            return read_ret;
        }
#else
        if (h->is_streamed && !s->reconnect_streamed)
