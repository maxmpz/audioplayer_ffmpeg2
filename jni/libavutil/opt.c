...
#if !PAMP_CHANGES // PAMP change - no video/image/color option parsing
...
#if !PAMP_CHANGES // PAMP change - no pixel format option parsing
...
#if !PAMP_CHANGES // PAMP change - no video/image option parsing
    ...
    case AV_OPT_TYPE_DURATION:
        {
            int64_t usecs = 0;
            if (val) {
#if !PAMP_CONFIG_NO_VIDEO // PAMP change - no av_parse_time dependency
...
#if !PAMP_CHANGES // PAMP change - no color option parsing
